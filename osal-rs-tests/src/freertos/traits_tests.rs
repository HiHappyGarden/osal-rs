...
//!
//! Mirrors `osal-rs/tests/std_traits_tests.rs`; keep the two in sync.
...
pub fn test_raw_mutex_guard_acquire() -> Result<()> {
...

    {
        ...
        // Held by this thread; the backing mutex is recursive, so the same
        // thread still sees it as lockable - assert on the unlock side below.
...
pub fn test_raw_mutex_guard_acquire_from_isr() -> Result<()> {
...
pub fn test_raw_mutex_guard_nested() -> Result<()> {
    log_info!(TAG, "Starting test_raw_mutex_guard_nested");

    // The backing mutex is recursive on both backends (FreeRTOS's
    // `xSemaphoreCreateRecursiveMutex`, POSIX's `PTHREAD_MUTEX_RECURSIVE`), so
    // nesting two guards on the same thread is legal and each drop unlocks
    // exactly one level.
    let mutex = leaked_raw_mutex();
...
pub fn test_mutex_guard_update() -> Result<()> {
...
pub fn test_mutex_guard_update_non_copy() -> Result<()> {
...
pub fn test_bytes_has_len_provided_method() -> Result<()> {
...
#[cfg(feature = "serde")]
pub fn test_bytes_has_len_blanket_impl() -> Result<()> {
...
pub fn test_thread_notification_into_tuple() -> Result<()> {
...
pub fn test_thread_metadata_default() -> Result<()> {
...
pub fn test_thread_state_variants() -> Result<()> {
...
pub fn test_to_priority_custom_impl() -> Result<()> {
...
pub fn run_all_tests() -> Result<()> {
    log_info!(TAG, "========== Running Traits Tests ==========");
    test_raw_mutex_guard_acquire()?;
    test_raw_mutex_guard_acquire_from_isr()?;
    test_raw_mutex_guard_nested()?;
    test_mutex_guard_update()?;
    test_mutex_guard_update_non_copy()?;
    test_bytes_has_len_provided_method()?;
    #[cfg(feature = "serde")]
    test_bytes_has_len_blanket_impl()?;
    test_thread_notification_into_tuple()?;
    test_thread_metadata_default()?;
    test_thread_state_variants()?;
    test_to_priority_custom_impl()?;
    log_info!(TAG, "========== All Traits Tests PASSED ==========");
    Ok(())
}
