...
//! Ported 1:1 from osal-rs-tests' FreeRTOS suite (`duration_tests.rs`) to run
//! against the POSIX backend. `posix::duration` has a real `TICK_PERIOD_MS`
//! (1ms/tick) backing these conversions, so this module is expected to pass.
...
#[test]
fn test_duration_to_ticks() -> Result<()> {
...
#[test]
fn test_duration_from_ticks() -> Result<()> {
...
#[test]
fn test_duration_conversion_roundtrip() -> Result<()> {
...
#[test]
fn test_duration_one_second() -> Result<()> {
...
#[test]
fn test_duration_microseconds() -> Result<()> {
...
#[test]
fn test_duration_large_value() -> Result<()> {
