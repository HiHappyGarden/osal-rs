...
//!
//! The mask is a single process-wide global, so every assertion lives in one
//! `#[test]` function: `cargo test` runs the tests of a binary on parallel
//! threads, and splitting these would let one test's `set_level_log` race
//! another's expectations. The original mask is restored at the end so the
//! rest of this binary (and any future test added to it) sees the default.
...
#[test]
fn test_log_mask_and_macros() -> Result<()> {
