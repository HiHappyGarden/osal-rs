...
//! Ported 1:1 from osal-rs-tests' FreeRTOS suite (`semaphore_tests.rs`) to
//! run against the POSIX backend. `posix::Semaphore` is currently a stub
//! (`wait`/`signal` always return `True`, no real counting) — assertions
//! that expect a timeout (`OsalRsBool::False`) fail until a real
//! implementation lands in `src/posix/semaphore.rs`.
...
#[test]
fn test_semaphore_creation() -> Result<()> {
...
#[test]
fn test_semaphore_signal_wait() -> Result<()> {
...
#[test]
fn test_semaphore_wait_timeout() -> Result<()> {
...
#[test]
fn test_semaphore_multiple_signals() -> Result<()> {
...
#[test]
fn test_semaphore_max_count() -> Result<()> {
...
#[test]
fn test_semaphore_initial_count() -> Result<()> {
...
#[test]
fn test_semaphore_binary() -> Result<()> {
